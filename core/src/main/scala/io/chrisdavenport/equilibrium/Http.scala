...
object Http {
  ...
  def servers[F[_]: MonadCancelThrow: Random](c: Config.Equilibrium, client: Client[F]): NonEmptyMap[Port, HttpRoutes[F]] = {
    val upstreams = c.http.upstreams.groupMapReduce(_.name)(_.servers){case (a, b) => a.concatNel(b)}
    c.http.servers.groupByNem(server => server.listen)
      .map{servers => 
        ...
        val routes: HttpRoutes[F] = HttpRoutes.of{ case (req: Request[F]) => 
          ...
          val exact = newServers.flatMap(_.locations).collect{case Location(out: Config.LocationMatcher.Exact, proxy) => (out, proxy)}
          
          val proxy = exact.collectFirst{ case (e, p) if e.value === pathRendered => p}
            .orElse{
              val prefix = newServers.flatMap(_.locations).collect{case Location(out: Config.LocationMatcher.Prefix, proxy) => (out, proxy)}
              prefix.collect{ case (e, p) if  pathRendered.startsWith(e.value) => (e, p)}
          ...
          proxy.fold(
            ...
            proxyThrough[F](_, upstreams).flatMap(uri => 
  ...
  private def proxyThrough[F[_]: Random : MonadThrow](proxyPass: String, upstreams: Map[String, NonEmptyList[Config.UpstreamServer]]): F[Uri] = {
    if (!proxyPass.contains("$")) {
        Uri.fromString(proxyPass).liftTo[F]
    } else {
      extractVariable(proxyPass).flatMap {
        case (before, variable, after) =>
          upstreams.get(variable).fold(throw new RuntimeException("Variable Not Found In Upstreams"))(nel => 
            pickUpstream[F](nel)
  ...
  private def extractVariable[F[_]: ApplicativeThrow](s: String): F[(String, String, String)] = {
    s.split('$').toList match {
      case before :: after :: _ => 
        val i = after.indexOf("/")
        if (i < 0) (before, after, "").pure[F]
        else after.split("/").toList match {
  ...
  private def pickUpstream[F[_]: Random: Monad](upstreams: NonEmptyList[Config.UpstreamServer]): F[Config.UpstreamServer] = {
  ...
  def xForwardedMiddleware[G[_], F[_]](http: Http[G, F]): Http[G, F] = Kleisli{ (req: Request[F]) => 
    req.remote.fold(http.run(req)){remote => 
      val forwardedFor = req.headers.get[`X-Forwarded-For`].fold(`X-Forwarded-For`(NonEmptyList.of(Some(remote.host))))(init => 
