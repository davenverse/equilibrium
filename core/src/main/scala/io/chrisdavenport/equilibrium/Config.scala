...
object Config {
  ...
  object Equilibrium {
    implicit val decoder: Decoder[Equilibrium] = new Decoder[Equilibrium]{
      def apply(c: HCursor): Decoder.Result[Equilibrium] = for {
        http <- c.downField("http").as[Http]
  ...
  object Location {
    def decoder(
      ...
      def apply(c: HCursor): Decoder.Result[Location] = for {
        modifier <- c.downField("modifier").as[Option[LocationMatcher.ModifierSymbol]].map(_.getOrElse(LocationMatcher.ModifierSymbol.Prefix))
        out <- modifier match {
        ...
        proxy <- c.downField("proxyPass").as[String](Decoder[String].emap{s => 
          if (!s.contains("$")) s.asRight
          else {
            s.split('$').lastOption.toRight("Value doesnt exist in split")
              .flatMap{ variable => 
                if (upstreams.contains(variable)) s.asRight
